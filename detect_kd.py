...
######################################################################
    ...
    while True:
        ...
        if draw_overlay:
            ...
            if len(objs) > 0:
                x0, y0, x1, y1 = objs[0].bounding_box.flatten().tolist()
                im = Image.fromarray(tensor[y0:y1, x0:x1])
                im = cv2.cvtColor(im, cv2.COLOR_RGB2BGR)
                objs2 = engine2.classify_with_image(im, threshold = 0.5, top_k = 10)
                print(objs2)
...
def add_render_gen_args(parser):
    ...
    parser.add_argument('--model2',
                        help='.tflite second model path', required=True)
