...
def overlay(title, objs, get_color, inference_time, inference_time2, inference_rate, layout, no_mask_count):
    ...
    for obj in objs:
        ...
        color = 'white' if obj.label == 'Mask' else 'red'
    ...
    lines = [
        'Objects: %d' % len(objs),
        'No mask: %d' % no_mask_count,
        'Inference time: %.2f ms (%.2f fps)' % (inference_time * 1000, 1.0 / inference_time),
        'Inference time mask : %.2f ms (%.2f fps)' % (inference_time2 * 1000, 1.0 / inference_time2)
    ]
...
def convert(obj, labels, scores, idx):
    x0, y0, x1, y1 = obj.bounding_box.flatten().tolist()
    return Object(id=idx,
                  label=labels[idx] if labels else None,
                  score=scores[idx] if scores else 0,
...
def render_gen(args):
    ...
    ###############################################################################
    engines2, titles2 = utils.make_engines(args.model2, ClassificationEngine)
    engines2 = itertools.cycle(engines2)
    engine2 = next(engines2)
    ###############################################################################
        ...
        if draw_overlay:
            ...
            if len(objs) > 0:
                ...
                for obj in objs:                
                    ...
                    x0, y0, x1, y1 = int(layout.inference_size[0] * x0), int(layout.inference_size[1] * y0), int(layout.inference_size[0] * x1), int(layout.inference_size[1] * y1)
                    
                    im = tensor

                    im = np.reshape(im, (W, H, 3))
                    im = Image.fromarray(im[y0:y1, x0:x1])                           
                    obj2 = engine2.classify_with_image(im, threshold=0.0, top_k = 10)
                 

                    mask = list(filter(lambda x: x[0] == 0, obj2))[0]
                    noMask = list(filter(lambda x: x[0] == 1, obj2))[0]
            ...
            inference_time2 = time.monotonic() - start2
            ####################################################################################            

            objs = [convert(obj, labels, scores, idx) for idx, obj in enumerate(objs)]
            ...
            title = titles[engine]
            output = overlay(title, objs, get_color, inference_time, inference_time2, inference_rate, layout, no_mask_count)
        else:
            output = None
...
def add_render_gen_args(parser):
    ...
    parser.add_argument('--model2', default=False,
                        help='.tflite model2 path', required=True)
